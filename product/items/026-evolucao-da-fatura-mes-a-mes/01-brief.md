...
morre. A curva distribui no tempo o que já está lançado; o mês de cada parcela é
decidido pelo dia de fechamento do cartão, que o item `024` passou a guardar.
...
- **Nenhum campo novo do cartão.** Limite, taxa, fechamento e vencimento vieram do
  `024` e são consumidos como estão.
...
- **RF-02.** O mês em que cada parcela cai é decidido pelo **dia de fechamento**
  do cartão: parcela lançada depois do fechamento entra na fatura seguinte.
- **RF-03.** Quando o cartão não tem dia de fechamento informado, a série é
  construída pelo mês do lançamento e a tela **declara essa premissa** em vez de
  fingir que sabe.
...
- **O dia de fechamento mudar o mês de uma parcela e ninguém notar.** Por isso
  RF-03 exige que a premissa seja declarada quando o dia não é conhecido.
