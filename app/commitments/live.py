...
def live_floor(today: date | None = None) -> str:
    # A floor, not a set of month labels: a card invoice arrives with instalments
    # dated months ahead, and a set of the current and previous month reads those
    # future charges as a series that stopped (RF-23). A floor lets the future in
    # and keeps the remote past out with one comparison.
...
def charged(conn: sqlite3.Connection, *, today: date | None = None) -> list[dict[str, Any]]:
    ...
    # charge the owner can already see on the invoice (RF-15).
    rows = conn.execute(
