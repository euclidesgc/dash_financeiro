...
def positions(conn: sqlite3.Connection) -> dict[str, int]:
    # Reason: three numbers, never one. Of the consolidated position, the
    # card share is debt at ~51% a year and the bank share is what falls into
    # the overdraft — a screen showing only the sum hides which of the two is
    # on fire (RF-04).
