...
def autenticar() -> str:
    ...
    with urllib.request.urlopen(req, timeout=60) as resposta:
        # apiKey é sempre string no contrato da Pluggy; json.loads devolve Any.
...
def extrair_oauth_url(item: dict[str, Any]) -> str | None:
    ...
    if isinstance(parametro, dict):
        if parametro.get("name") in ("oauthUrl", "oauth"):
            # "data"/"value" da Pluggy são sempre string ou ausentes; dict[str, Any]
            # torna a leitura Any para o verificador.
