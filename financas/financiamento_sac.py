...
def main():
    ...
    print(f"  primeira parcela     R$ {base['primeira']:>10,.2f}   (observado no extrato: R$ {ENCARGO_OBSERVADO:,.2f})")
    ...
    print(f"  seguros/taxas fora do calculo: R$ {seguros:,.2f}/mes -> R$ {seguros*MESES:,.2f} em {MESES} meses")
    print()

    liq = DADOS["liquidacao_recursos_proprios"]["valor_total_da_divida"]
    print("=== LIQUIDANDO AGORA ===")
    print(f"  valor da liquidacao em {DADOS['liquidacao_recursos_proprios']['data_prevista']}: R$ {liq:>10,.2f}")
    print(f"  economia nominal de juros:  R$ {base['juros_total'] - (liq - SALDO):>10,.2f}")
    print(f"  (a liquidacao custa R$ {liq - SALDO:,.2f} acima do saldo — sao as 2 prestacoes em aberto e encargos)")
    print()
    ...
    for extra in (5000, 10000, 20000, 50000):
        ...
        print(f"  R$ {extra:>7,}  {s['parcelas']:>5}m  {MESES - s['parcelas']:>11}m  "
    ...
    print(f"  cheque especial Itau       {100*juros_ce_itau/saldo_ce_itau:>6.2f}% a.m.  "
    ...
    print()
    print(f"  R$ 10.000 no cheque especial evitam  R$ {10000*juros_ce_itau/saldo_ce_itau:>8,.2f}/mes")
