...
PMT = D["valor_parcela"]
I = D["juros_efetivo_mensal_pct"] / 100
N = D["prazo_meses"]
HOJE = date(2026, 9, 5)
...
def main():
    ...
    print(f"principal R$ {D['principal']:,.2f} · {N}x R$ {PMT:,.2f} · {D['juros_efetivo_mensal_pct']}% a.m. "
          f"({D['juros_efetivo_anual_pct']}% a.a.) · CET {D['cet_anual_pct']}% a.a.")
    print()

    pv60 = valor_presente(N, PMT, I)
    ...
    print(f"  -> as tarifas e o IOF foram financiados junto: diferenca R$ {pv60 - D['principal']:,.2f}")
    print()

    pagas = parcelas_vencidas()
    restantes = N - pagas
    saldo = valor_presente(restantes, PMT, I)
    nominal = restantes * PMT
    print(f"=== POSICAO EM {HOJE.strftime('%d/%m/%Y')} ===")
    print(f"  parcelas pagas:      {pagas} de {N}  (ultima vencida: 11/08/2026)")
    print(f"  parcelas restantes:  {restantes}  (de 11/09/2026 a {D['ultimo_vencimento'][8:10]}/"
          f"{D['ultimo_vencimento'][5:7]}/{D['ultimo_vencimento'][:4]})")
    print(f"  a pagar nominal:     R$ {nominal:>10,.2f}")
    print(f"  SALDO DEVEDOR hoje:  R$ {saldo:>10,.2f}   (valor presente a {I*100:.2f}% a.m.)")
    ...
    print(f"  (o banco e obrigado a dar o desconto proporcional dos juros nao corridos)")
    print()
    ...
    for nome, taxa, obs in linhas:
        print(f"  {nome:<28} {taxa:>5.2f}% a.m.  R$ 10.000 economizam R$ {10000*taxa/100:>7,.2f}/mes   ({obs})")
