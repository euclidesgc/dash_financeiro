...
- **D4** — `app/sync/fetch.py` importa `consolidate` e `NoRawAccountsError` e apanha só `NoRawAccountsError`; nenhum `SystemExit` resta no serviço (`app/sync/fetch.py`, `app/sync/__init__.py`).
