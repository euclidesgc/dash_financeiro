...
- [ ] T1.1 — Precedência em SQL
  ...
  - Complexidade: baixa
- [ ] T1.2 — `display_name` lê a consulta
  ...
  - Complexidade: baixa
- [ ] T1.3 — Lista de gastos junta a consulta
...
- [ ] CA1.3 — `grep -rn "merchant_name" app/payees app/queries/expenses.py` não encontra nada; a ordem das origens aparece uma vez só, em `app/queries/payees.py`. (estrutural)
- [ ] CA1.4 — O `payee_name` de cada item da lista é o `name` de `display_name` para o mesmo `payee` (ou nulo quando a origem é `descricao`), e a busca `q` acha um gasto pelo nome exibido e não pelo nome de nível inferior; inverter duas origens na tupla faz os testes de precedência falharem nas duas telas. (comportamental)
