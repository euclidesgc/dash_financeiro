...
- **D3** — O import de `app.sync.connections` no topo de `ingestao/pluggy_extract.py` não cria ciclo que quebre: `app/sync/__init__.py` importa `app.sync.fetch`, que importa `ingestao.pluggy_extract`, que importa o submódulo `app.sync.connections`, carregável com o pacote `app.sync` ainda parcial porque ele só depende de `app.db` e `app.migrate`.
...
- `ingestao/pluggy_extract.py` (alterar)
