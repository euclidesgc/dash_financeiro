...
- [x] T1.1 — Utilitário compartilhado
...
- [x] CA1.3 — `grep -rnE "=== 1 \?|count === 1\)" src --include=*.ts --include=*.tsx` fora de `__tests__/` e de `src/utils/format-count.ts` não encontra nada. (estrutural)
- [x] CA1.4 — Trocar em `formatCount` a condição do singular para `count <= 1` faz falhar o teste do zero em `format-count.test.ts`; desfeita a troca, volta a passar. (comportamental)
