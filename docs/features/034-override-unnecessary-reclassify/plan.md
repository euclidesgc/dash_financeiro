...
- [ ] T1.1 — Escrita com reclassificação opcional
...
- [ ] CA1.3 — Em `app/taxonomy/override.py`, `set_not_expense` e `clear_not_expense` chamam `_write` com `reclassify=False`, e os outros três escritores com `reclassify=True`. (estrutural)
- [ ] CA1.4 — Passar `reclassify=True` em `set_not_expense` faz falhar o teste do grupo desatualizado em `tests/test_override.py`; desfeita a troca, volta a passar. (comportamental)
