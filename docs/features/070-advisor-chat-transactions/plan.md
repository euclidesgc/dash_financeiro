...
- [x] T1.1 — Filtro por categoria e formatação de reais fora do router
...
- [x] T1.2 — Porta de provedor, adaptadores Anthropic e Gemini e seleção
...
- [x] T1.3 — Ferramenta, laço, persistência e rotas
...
- [x] CA1.1 — (estrutural) `app/migrations/sql/026_advisor_chat.sql` cria `advisor_conversations` e `advisor_messages` com `role IN ('user','assistant','tool')`.
- [x] CA1.2 — (estrutural) `app/routers/advisor_chat.py` não contém SQL nem `commit` (G7) e as rotas são `def`.
- [x] CA1.3 — (comportamental) Com um provedor falso que pede `search_transactions` e depois responde, `POST /api/advisor/conversations/{id}/messages` devolve a resposta e grava pergunta, pedido, resultado e resposta; provado em `tests/test_advisor_chat.py`.
- [x] CA1.4 — (comportamental) Sem chave, `GET /api/advisor/status` devolve `available: false` e o envio devolve 503 com o texto de configuração; sem sessão, toda rota `/api/advisor/*` devolve 401.
- [x] CA1.5 — (comportamental) Resposta com valor em reais que nenhuma ferramenta devolveu é trocada pelo aviso; o laço para em 8 voltas.
- [x] CA1.6 — (comportamental) Os adaptadores traduzem pedido de ferramenta e resultado nos dois sentidos, reenviam os blocos crus ao mesmo provedor e traduzem erro de chave, excesso e rede em pt-BR, sem rede nos testes.
- [x] CA1.7 — (comando) `bash scripts/lint.sh`, `bash scripts/gates/gates_runner.sh` e `uv run pytest` passam.

## Fase 2 — Tela do consultor na SPA

- [x] T2.1 — Tipos, chamadas e mock
  - Arquivos: `src/features/advisor/types/advisor.ts`, `src/features/advisor/api/*.ts` (criar); `src/testing/mocks/handlers.ts` (alterar)
  - Complexidade: baixa

- [x] T2.2 — Tela e navegação
...
- [x] CA2.1 — (comportamental) O cabeçalho mostra "Consultor" e leva a `/app/advisor`.
- [x] CA2.2 — (comportamental) Enviar pergunta mostra "Consultando suas contas…", depois a resposta; a conversa reaparece ao recarregar; "Nova conversa" limpa a tela.
- [x] CA2.3 — (comportamental) Sem provedor, a tela mostra como configurar e não mostra o campo de pergunta.
- [x] CA2.4 — (comportamental) Erro do provedor aparece em alerta com o texto do servidor e o campo mantém a pergunta.
- [x] CA2.5 — (comando) `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build` e `pnpm test:e2e` passam.
