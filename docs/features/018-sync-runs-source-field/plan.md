...
- [x] CA1.3 — `synchronise` e `ingest` não têm valor padrão para `trigger`; `grep -rn "trigger=" app/` mostra `COMMAND` em `app/sync/__init__.py` (`main`) e `app/ingest/__main__.py`, e `SCREEN` (ou `trigger.SCREEN`) em `app/routers/sync.py` e `app/routers/summary.py`. (estrutural)
- [x] CA1.4 — Depois de `POST /api/sync/run`, `GET /api/sync/status` devolve `last_run.triggered_by == "screen"`; `main()` do comando grava `command`; uma falha de fonte grava a origem de quem pediu; uma linha sem origem sai como `null`; o banco recusa `triggered_by = 'outro'`. (comportamental)
- [x] CA1.5 — O painel mostra "Pedida na tela" para `screen`, "Feita pela rotina diária" para `command` e nenhum dos dois para `null`. (comportamental)
