...
- [ ] T1.1 — Consulta `category_labels`
  ...
  - Complexidade: baixa
- [ ] T1.2 — Routers sem `LABELS` de import
...
- [ ] CA1.3 — `grep -rn "seed_labels\|LABELS" app/routers/spending.py app/routers/rules.py` não devolve nada; `category_labels` está em `app/queries/categories.py`. (estrutural)
- [ ] CA1.4 — Comportamento: depois de `UPDATE categories SET label = …` com o servidor já no ar, a tabela, o painel e a lista aberta de `/gastos` e a tela `/regras` mostram o rótulo novo; uma categoria inserida em `categories` com rótulo próprio aparece com esse rótulo nas duas telas; voltar a ler `seed_labels()` nos routers faz esses testes falharem. (comportamental)
