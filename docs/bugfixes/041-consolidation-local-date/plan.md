...
- [ ] T1.1 — Conversão do instante para o dia local
  - Arquivos: `ingestao/raw.py` (alterar), `ingestao/pluggy_consolidate.py` (alterar), `ingestao/pluggy_extract.py` (alterar)
  - O que fazer: `local_date(instant: str | None) -> str` devolve `""` para ausência, converte instante com fuso para `America/Sao_Paulo` e devolve o `date().isoformat()`; instante sem fuso é tomado como local. A coluna `data` da consolidação e a primeira e última data do inventário passam a usá-la.
  - Skills: python-tipagem-estrita, python-ruff
  - Complexidade: baixa
- [ ] T1.2 — Reconciliação dos números de referência
...
- [ ] CA1.1 — `uv run pytest tests/test_pluggy_scripts.py -k sao_paulo` sai com código 0, e o mesmo comando antes da correção falhava em `2026-04-01T02:59:00.000Z` e `2026-09-01T01:25:35.091Z`. (comando)
- [ ] CA1.2 — `bash scripts/lint.sh`, `bash scripts/gates/gates_runner.sh` e `uv run pytest` saem com código 0. (comando)
- [ ] CA1.3 — Nenhum `[:10]` sobre data da Pluggy em `ingestao/`. (estrutural)
- [ ] CA1.4 — Consolidando o bruto de 05/09/2026, o fluxo de 03/2026 a 08/2026 dá gasto R$ 104.197,31 e déficit de 04 a 08/2026 de R$ 5.661,30/mês, e `docs/plano.md` traz esses valores ao lado dos antigos. (comportamental)
- [ ] CA1.5 — Depois de consolidar e carregar a base real pelo caminho normal, o salário de R$ 2.928,46 aparece em 31/03/2026 no banco. (comportamental)
