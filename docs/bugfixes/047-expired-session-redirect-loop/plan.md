...
- [ ] T1.1 — Usuário ausente como resposta
  ...
  - Complexidade: baixa
- [ ] T1.2 — Testes
  - Arquivos: `src/app/__tests__/login-to-balances.test.tsx`, `src/lib/__tests__/auth.test.ts` (criar)
  - O que fazer: o teste de regressão passa; `getMe` devolve `null` no 401 e rejeita com o erro em outro status.
  - Skills: component-testing, unit-testing
  - Complexidade: baixa

### Critérios de aceite da fase 1

- [ ] CA1.1 — `pnpm test` sai com código 0, e no commit `c94193e` o teste `a session that expires while the app is open lands on the login page and stays there` falhava. (comando)
- [ ] CA1.2 — `pnpm lint`, `pnpm typecheck`, `pnpm build` e `pnpm test:e2e` saem com código 0. (comando)
- [ ] CA1.3 — Em `src/lib/auth.tsx`, `getMe` devolve `Promise<User | null>`, só o status 401 vira `null`, e `ProtectedRoute` não consulta mais o status do erro para redirecionar. (estrutural)
- [ ] CA1.4 — `getMe` devolve `null` quando `/api/auth/me` responde 401 e rejeita com `ApiError` de status 500 quando a API responde 500. (comportamental)
- [ ] CA1.5 — No navegador, com a sessão encerrada no servidor, voltar o foco à aba dos saldos leva a "Entrar" e a URL fica em `/app/login`; entrar de novo mostra "Saldos de hoje". (comportamental)
