...
- [ ] T1.1 — Faixa do SQLite e leitor único
  ...
  - Complexidade: baixa
- [ ] T1.2 — Rotas da API com a faixa na entrada
  - Arquivos: `app/routers/transactions.py`, `app/routers/rules.py` (alterar)
  - O que fazer: id de lançamento e id de regra como `Annotated[int, Path(ge=SQLITE_INTEGER_MIN, le=SQLITE_INTEGER_MAX)]`; `page` com `le=SQLITE_INTEGER_MAX // MAX_PAGE_SIZE`; `_number` usa `storable_int`.
  - Skills: python-dependencies-para-validacao
  - Complexidade: baixa
- [ ] T1.3 — Teto e limite com o maior valor aceito
  ...
  - Complexidade: baixa
- [ ] T1.4 — Telas antigas e fim do mês
...
- [ ] CA1.1 — `uv run pytest` sai com código 0, e no commit `e022fc3` os testes de `tests/test_integer_range.py` e `test_the_last_month_the_calendar_has_ends_on_its_last_day` falhavam. (comando)
- [ ] CA1.2 — `bash scripts/lint.sh` e `bash scripts/gates/gates_runner.sh` saem com código 0. (comando)
- [ ] CA1.3 — A faixa do SQLite aparece como número só em `app/db.py`; `app/routers/transactions.py`, `app/routers/rules.py`, `app/routers/debts.py` e `app/routers/spending.py` não chamam `int()` sobre texto da requisição. (estrutural)
- [ ] CA1.4 — `PUT /api/plan/ceiling` com 1.000.000.000.000 centavos responde 422 com `O teto passa do maior valor aceito, R$ 9.999.999.999,99.`; com 999.999.999.999 responde 200. (comportamental)
