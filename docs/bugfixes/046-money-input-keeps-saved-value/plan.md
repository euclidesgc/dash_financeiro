...
- [ ] T1.1 — Leitura do valor em formato brasileiro
  - Arquivos: `src/utils/money-text.ts` (criar), `src/utils/money-text-schema.ts` (alterar), `src/utils/cents.ts` (remover), `src/features/expenses/types/ceiling-schema.ts` e `src/features/categories/types/category-limit-schema.ts` (tipos de entrada e saída)
  - O que fazer: `parseMoneyText` e `formatMoneyInput`; o esquema transforma em centavos com uma mensagem por motivo.
  - Skills: forms, code-standards
  - Complexidade: baixa
- [ ] T1.2 — Formulários e painel
...
- [ ] CA1.1 — `pnpm test` sai com código 0, e no commit `7a33940` os seis testes de regressão listados em `investigation.md` falhavam. (comando)
- [ ] CA1.2 — `pnpm lint`, `pnpm typecheck`, `pnpm build` e `pnpm test:e2e` saem com código 0. (comando)
- [ ] CA1.3 — Em `ceiling-form.tsx` e `category-limit-form.tsx` o campo é `type="text"` com `inputMode="decimal"`, e o único ponto que manda `null` à API é o botão de remoção. (estrutural)
- [ ] CA1.4 — `parseMoneyText` devolve 150000 para "1.500,00", "1500", "1.500", "R$ 1.500,00" e "1500,00"; 150050 para "1500,5" e "1500.50"; e recusa "", "abc", "15,00,0", "1,999" e "-5" com o motivo certo. (comportamental)
- [ ] CA1.5 — No navegador, com teto gravado, digitar "1.500,00" e salvar mostra "de R$ 1.500,00" no painel; o botão diz "Alterar teto"; "Remover teto" volta a "Sem teto definido". (comportamental)
