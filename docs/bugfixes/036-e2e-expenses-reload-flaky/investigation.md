...
- Com a correção: `pnpm exec playwright test e2e/expenses.spec.ts --repeat-each=20` deu 220 de 220, e o teste do período passou nas 40 execuções de duas rodadas.
...
- **Fora da correção:** nenhuma outra tela usa `setSearchParams`. Rodar a suíte e2e inteira com `--repeat-each` para caçar a instabilidade mostrou que `e2e/sync.spec.ts` não aguenta segunda volta na mesma base (espera "Nunca atualizado"); vira o item 037 do roadmap. Em duas rodadas de `--repeat-each=20` de `e2e/expenses.spec.ts`, já com a correção, o teste do teto do mês falhou uma vez em 40 depois do reload, sem relação com a URL; vira o item 038.
