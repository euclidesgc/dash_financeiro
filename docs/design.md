...
| Cabeçalho de app | `<header className="border-b border-gray-200">` com `mx-auto flex max-w-2xl flex-wrap items-center justify-between gap-x-4 gap-y-2 p-4`; filhos diretos, nesta ordem: nome do painel, navegação e bloco do usuário (`flex min-w-0 items-center gap-4`, login em `truncate`, ação à direita); cabe em 375 px sem rolagem horizontal | 001, 052 |
...
| Navegação do cabeçalho | `<nav aria-label="Principal" className="order-last flex w-full flex-wrap items-center gap-x-4 gap-y-2 sm:order-none sm:mr-auto sm:w-auto">`: no celular, segunda linha inteira abaixo do nome e do usuário; a partir de `sm`, ao lado do nome, empurrando o usuário para a direita; `NavLink` com `text-sm font-medium underline-offset-4 hover:underline`; ativo (`aria-current="page"`) `text-gray-900 underline`; inativo `text-gray-600` | 003, 052 |
...
| Painel de resultado | `<section>` com `<h2 className="mt-6 text-lg font-semibold">` e, abaixo, o período a que se refere em `<p className="mt-1 text-sm text-gray-600">`; `<dl className="mt-2 flex flex-wrap gap-x-8 gap-y-2 rounded-md border border-gray-200 p-4">`; cada par em `<div className="min-w-0">` com `<dt className="text-sm text-gray-600">` e `<dd className="tabular-nums font-medium">` mais a cor do sinal | 016, 054 |
| Página de gastos | ordem: título, resultado do período, teto do mês, barra de controles, total por categoria, lista; sem período na URL a página mostra o mês corrente, e "Todo o período" é `period=all` | 005, 054 |
| Valor de entrada | par do "Valor monetário" para positivo que é receita: `text-green-700` | 016 |
| Identificador técnico | `font-mono text-sm text-gray-900` com `truncate` e `title` com o valor inteiro; o `<input>` que o recebe usa a receita "Campo de formulário" mais `font-mono text-sm min-h-10`, com dica em `text-sm text-gray-600` abaixo | 021 |
| Cartão do plano do mês | "Painel de resultado" com a borda num `<div className="mt-2 rounded-md border border-gray-200 p-4">` que envolve o `<dl className="flex flex-wrap gap-x-8 gap-y-2">` (o primeiro `<dd>` com o selo de status ao lado, `flex flex-wrap items-center gap-2`), uma linha `<p className="mt-3 text-sm tabular-nums">` (`text-gray-600` para folga ou sem teto, `text-red-800` para excesso) e o "Link de navegação" com `mt-3 inline-block` | 055 |
| Busca em lista carregada | `<div role="search" className="mt-6 flex flex-col gap-1">` com rótulo e `<input type="search">` da receita "Campo de formulário" mais `min-h-10`; botão secundário "Limpar" ao lado enquanto houver texto; filtra na hora, sem distinguir acento nem maiúscula; sem resultado, a receita "Vazio" dizendo o termo e o próximo passo; acima da lista, resumo da ordem em `mt-4 text-sm text-gray-600` com números `tabular-nums` | 058 |
| Conversa | `<ol aria-label="Conversa" className="mt-6 flex flex-col gap-4">`; mensagem do dono à direita em `flex justify-end` com balão `max-w-[85%] rounded-md bg-blue-50 px-4 py-3 text-gray-900 whitespace-pre-wrap break-words`; resposta à esquerda com o mesmo balão trocando o fundo por `border border-gray-200`, e abaixo `mt-1 text-xs text-gray-600` dizendo quem respondeu; cada balão começa por um `sr-only` ("Você: ", "Consultor: "); enquanto espera, a pergunta aparece com `opacity-70` e a receita "Carregando" diz "Consultando suas contas…" | 070 |
| Aviso de configuração | `<section className="mt-6 rounded-md border border-amber-200 bg-amber-50 p-4">` com `<h2 className="font-semibold text-amber-900">`, texto `mt-2 text-amber-900` e passos em `<ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-amber-900">`; nome de variável em `<code className="font-mono">` | 070 |
| Cartão de proposta no chat | abaixo da resposta do consultor, `<section aria-labelledby>` com `mt-2 w-full max-w-[85%] rounded-md border border-gray-200 p-4`; título `h3` `font-medium` com o selo de status à direita (âmbar pendente, verde aplicada, cinza descartada ou desfeita); lista em `mt-3 max-h-80 overflow-y-auto divide-y divide-gray-200` com a "Linha de lançamento" e "atual → nova" em `text-sm text-gray-600`; total em `border-t pt-3`; resultado em `<p role="status" className="mt-3 text-sm text-gray-600">`; botões principal e secundário abaixo | 072 |
