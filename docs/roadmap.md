...
| 024 | `payee-filling-at-ingest` | preencher `payee` na própria `ingest`, na transação da carga, para que toda base (sincronização, testes, e2e) nasça com recebedor; a classificação segue no pós-carga | 003 | — | in-progress |
...
| 032 | `e2e-seed-classifies` | o recebedor já nasce preenchido na carga, mas o seed de `scripts/e2e-backend.sh` não roda `classify_all`, então grupo, natureza e essencialidade ficam sem classificação automática; aplicar a classificação no seed | 011 | 024 | planned |
