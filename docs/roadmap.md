...
| 034 | `override-unnecessary-reclassify` | `app/taxonomy/override.py::_write` reclassifica tudo via `classify_all` mesmo quando a coluna escrita (`not_expense_reason`) não é lida pela classificação; remover custo desnecessário a cada marcação de não-gasto | 015 | — | in-review |
