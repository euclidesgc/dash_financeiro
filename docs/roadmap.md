...
| 037 | `e2e-suite-repeatable` | rodar `pnpm test:e2e --repeat-each=N` na suíte inteira para caçar teste instável; hoje a segunda volta de `e2e/sync.spec.ts` ("updates the records from the balances page") falha porque espera "Nunca atualizado" numa base que a primeira volta já sincronizou | 036 | — | in-progress |
