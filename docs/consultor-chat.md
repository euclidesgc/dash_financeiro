...
| `debts_by_liquidity` | opcional, valor disponível agora em centavos e limite da lista (1 a 50, padrão 20) | dívidas e compras parceladas com parcela a vencer ordenadas pela parcela liberada por mês a cada real pago (parcela ÷ valor para quitar hoje), com valor para quitar, parcela liberada, parcelas que faltam e mês da última; o valor pela soma das parcelas que faltam marcado "estimativa pelo teto — informe o saldo de quitação"; à parte, cheque especial, saldo de cartão e contrato sem parcela; com o valor disponível, o que quitar na ordem da lista pulando o que não cabe, o total gasto, a sobra e a parcela liberada no total | `app/debts/payoff.py::rank_by_liquidity`, sobre `list_debts` e `payoff_at` da 074, que leem a parcela e o fim da mesma fonte da projeção da 073 (`charged` e o contrato) | 075 |
